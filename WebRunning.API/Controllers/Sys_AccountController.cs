...
using FacebookCore;
using WebRunning.InfraLib.Email;
    ...
    {        
        ...
        public async Task<IActionResult> Login([FromBody] LoginRequest request)
        {
            try
            {
                ...
                var result = await _service.Sys_User.CheckUserLogin(request.UserName, request.Password, request.Type);
                var claims = new[]
                {
        ...
        public async Task<IActionResult> Signup([FromBody] SignupRequest request)
        {
            try
            {
                ...
                if (userNew.UserName == "admin") { 
                    ...
                    userNew.Type = Lib.Enums.UserType.System;
                }
                else
                {
        ...
        private async Task SendNoti_RegisteredUsers(Guid userId, string email)
        {
            try
            {
                ...
                oNotification.Type = Lib.Enums.System.NotificationType.Info;
        ...
        public async Task<IActionResult> ChangePassword([FromBody] ChangePasswordRequest request)
        {
            try
            {
                ...
                var user = await _service.Sys_User.CheckUserLogin(_userProvider.UserName, request.PasswordOld, Lib.Enums.UserType.Public);
        ...
        public async Task<IActionResult> SendEmailRestorePassword([FromBody] SendEmailRestorePasswordRequest inputRequest)
        {
            try
            {
                ...
                emailService.Send(inputRequest.Email, "WebRunning khôi phục lại mật khẩu", "link khôi phục: " + inputRequest.Address + "?code=" + code, ref errorMessage);
        ...
        [HttpPost("Signupportal")]
        [AllowAnonymous]
        public async Task<IActionResult> Signupportal([FromBody] SignupRequest request)
        {
            try
            {
                _logger.LogInformation(string.Format("Call Signupportal body: ({0})", JsonConvert.SerializeObject(request)));
                if (!UserHelpers.IsValidUserLogin(request.UserName, request.PassWord, out string message))
                {
                    throw new Exception(message);
                }
        ...
        public async Task<IActionResult> Loginportal([FromBody] LoginRequest request)
        {
            try
            {
                _logger.LogInformation(string.Format("Call Loginportal body: ({0})", JsonConvert.SerializeObject(request)));
                if (!UserHelpers.IsValidUserLogin(request.UserName, request.Password, out string message))
                {
                    throw new Exception(message);
                }
