...
namespace WebRunning.API.Controllers
{
    public class Sys_CategoryController : ApiControllerBase<Sys_Category>
    {
        ...
        [AuthorizeFilter]
        public async Task<IActionResult> GetListEntitiesByTypeAsync(int page = 1, int pageSize = 10, int totalLimitItems = 500, int type = 0)
        {
            try
            {
                _logger.LogInformation(string.Format("Call GetListEntitiesByTypeAsync params: (page = {0}, pageSize = {1}, totalLimitItems = {2}, type = {3})", page, pageSize, totalLimitItems, type));
                string search = $"type = {type}";
                var items = await _service.Sys_Category.GetListEntitiesAsync(page, pageSize, totalLimitItems, search);
                return ResponseMessage.Success(items);
            }
            catch (Exception ex)
            {
                _logger.LogError(string.Format("GetListEntitiesByTypeAsync : {0}", ex.Message));
