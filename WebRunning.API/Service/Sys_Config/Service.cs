...
namespace WebRunning.API.Service.Sys_Config
{
    public class Service:RepositoryBase<Model.Sys_Config>, Sys_Config.IService
    {
        ...
        public async Task<bool> IsDupicateAttributesAsync(Guid? Id, string Code, int Type)
        {
            ...
            {
                result = await _dbContext.Sys_Configs.Where(o => o.Code == Code && o.Type == (Lib.Enums.ConfigType)Type).AnyAsync();
            }
            else
            {
                var items = await _dbContext.Sys_Configs.Where(o => o.Code == Code && o.Type == (Lib.Enums.ConfigType)Type).ToListAsync();
                if (items.Count >= 1)
                {
                    if (items.Count == 1)
                    {
