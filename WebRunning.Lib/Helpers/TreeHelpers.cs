...
namespace WebRunning.Lib.Helpers
{
    public class TreeHelpers<T> where T : absTree<T>
    {
        ...
        public static List<T> GetChildNodesByNodeParent(T parentNode)
        {
            List<T> childNodes = new List<T>();
            GetChildNodesRecursive(parentNode, parentNode, childNodes);
            return childNodes;
        }

        private static void GetChildNodesRecursive(T node, T parentNode, List<T> childNodes)
        {
            ...
            if (node.Children != null)
            {
                ...
                {
                    GetChildNodesRecursive(node.Children[i], parentNode, childNodes);
                }
            }
        }
